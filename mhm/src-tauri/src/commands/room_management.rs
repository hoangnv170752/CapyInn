...
#[tauri::command]
pub async fn create_room(state: State<'_, AppState>, app: tauri::AppHandle, req: CreateRoomRequest) -> Result<Room, String> {
    require_admin(&state)?;

    // Check ID doesn't already exist
    let existing: Option<(String,)> = sqlx::query_as("SELECT id FROM rooms WHERE id = ?")
        .bind(&req.id).fetch_optional(&state.db).await.map_err(|e| e.to_string())?;
    if existing.is_some() {
...
#[tauri::command]
pub async fn delete_room(state: State<'_, AppState>, app: tauri::AppHandle, room_id: String) -> Result<(), String> {
    require_admin(&state)?;

    // Check room exists
    let room_row = sqlx::query("SELECT status FROM rooms WHERE id = ?")
        .bind(&room_id).fetch_optional(&state.db).await.map_err(|e| e.to_string())?;
    ...
    let active: (i64,) = sqlx::query_as(
        "SELECT COUNT(*) FROM bookings WHERE room_id = ? AND status = 'active'"
    ).bind(&room_id).fetch_one(&state.db).await.map_err(|e| e.to_string())?;
    if active.0 > 0 {
        return Err("Không thể xóa phòng có booking đang hoạt động".to_string());
    }

    sqlx::query("DELETE FROM rooms WHERE id = ?")
        .bind(&room_id).execute(&state.db).await.map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn create_room_type(state: State<'_, AppState>, app: tauri::AppHandle, req: CreateRoomTypeRequest) -> Result<RoomType, String> {
    require_admin(&state)?;
    ...
    sqlx::query("INSERT INTO room_types (id, name, created_at) VALUES (?, ?, ?)")
        .bind(&id).bind(&req.name).bind(&now)
    ...
    Ok(RoomType { id, name: req.name, created_at: now })
}

#[tauri::command]
pub async fn delete_room_type(state: State<'_, AppState>, app: tauri::AppHandle, room_type_id: String) -> Result<(), String> {
    require_admin(&state)?;

    // Check no rooms using this type
    let rt_row = sqlx::query("SELECT name FROM room_types WHERE id = ?")
        .bind(&room_type_id).fetch_optional(&state.db).await.map_err(|e| e.to_string())?;
    ...
    let in_use: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM rooms WHERE type = ?")
        .bind(&type_name).fetch_one(&state.db).await.map_err(|e| e.to_string())?;
    if in_use.0 > 0 {
        return Err(format!("Không thể xóa: có {} phòng đang sử dụng loại '{}'", in_use.0, type_name));
    }

    sqlx::query("DELETE FROM room_types WHERE id = ?")
        .bind(&room_type_id).execute(&state.db).await.map_err(|e| e.to_string())?;
...
pub async fn export_csv(state: State<'_, AppState>) -> Result<String, String> {
    ...
    for r in &guests {
        csv2.push_str(&format!("{},{},{},{},{}\n",
            ...
            r.get::<Option<String>, _>("nationality").unwrap_or_default(),
